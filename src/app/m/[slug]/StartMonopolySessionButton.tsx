...
export function StartMonopolySessionButton({ slug }: { slug: string }) {
    ...
    <div className="flex flex-col gap-2">
      ...
      <button
        ...
        onClick={async () => {
          setError(null);
          setPending(true);
          const res = await fetch(`/api/m/${encodeURIComponent(slug)}/session`, { method: "POST" });
          setPending(false);
          if (!res.ok) {
            setError("Could not start session. Try again.");
            return;
          }
          router.push(`/m/${slug}/allocate`);
        }}
