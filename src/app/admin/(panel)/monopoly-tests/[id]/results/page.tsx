...
export default async function MonopolyResultsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <Link href={`/admin/monopoly-tests/${id}`} className="text-sm text-neutral-600 hover:underline dark:text-neutral-400">
            ← {exercise.name}
          </Link>
        ...
        <a
          href={`/admin/monopoly-tests/${id}/results/export.csv`}
          ...
          <table className="w-full min-w-[480px] text-left text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-950">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">Bills placed</th>
              </tr>
            </thead>
            <tbody>
              {exercise.participantSessions.map((s) => (
                <tr key={s.id} className="border-b border-neutral-100 dark:border-neutral-800">
                  ...
                  <td className="px-4 py-2">{s._count.allocations}</td>
                </tr>
              ))}
            </tbody>
          </table>
